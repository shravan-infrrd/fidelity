...
def xml_total_cs(xml_path):
		sections = get_sections(xml_path)
		stocks = []
		for i, sec in enumerate(sections):
				stock_values = []
				data = {}
				if sec['ICLRP001-PREFIX-RECORD']['ICLRP001-RECORD-SUB-TYP1'] == 'STK':
						if sec['ICLRP001-PREFIX-RECORD']['ICLRP001-RECORD-SUB-TYP2'] == 'CS':
								stocks.append(sec['ICLR0203-EXTRACT-VIEW']['ICLR0203-HLD-BB-MKT-VAL'])
								stocks.append(sec['ICLR0203-EXTRACT-VIEW']['ICLR0203-HLD-MKT-VALUE'])
								stocks.append(sec['ICLR0203-EXTRACT-VIEW']['ICLR0203-EST-ANNUAL-INCOME'])

		return stocks

def get_stocks_data(xml_path=""):
		print(f"get_stocks_data ***** CALLED***{xml_path}")
		sections = get_sections(xml_path)
		stocks = []
		for i, sec in enumerate(sections):
				stock_values = []
...
def get_preferred_stocks_data( xml_path):
